...
func setupFlags(cmd *cobra.Command) error {
	...
	dataDir := path.Join(os.TempDir(), "distdb")
	cmd.Flags().String("data-dir", dataDir, "Directory to store store and raft data.")
	...
	cmd.Flags().StringSlice("start-join-addrs", nil, "Serf addressses to join.")
	cmd.Flags().Bool("bootstrap", false, "Bootstrap the cluster.")
	cmd.Flags().String("acl-model-file", "", "Path to ACL model.")
	cmd.Flags().String("acl-policy-file", "", "Path to ACL file.")
